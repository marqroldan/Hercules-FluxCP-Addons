...
class CharacterRender extends RORender
{
	...
	public function changeData($data) {
		...
		$this->param['background'] = $this->param['bg_loc']."background/".$this->param['background'];
		$this->param['border'] = $this->param['bg_loc']."border/".$this->param['border'];
		
		$this->body_animation = $this->param['body_animation'];
		$this->doridori = $this->param['doridori'];
		$this->action = $this->param['action_'];
		$this->direction = $this->param['direction'];
	}
	
	public function render($sig)
	{
		...
		*/
		...
		/////// $img is the render variable of the character
		...
		if ($sig) {
			...
			$status_color = $view['online'] ? imagecolorallocate($output, 125,  196,  126 ) : imagecolorallocate($output, 190,  83,  83 );
			...
			// Draw text
			imagestringright( $output, 2, 187, 69, $view['name'], $name_color );
			imagestringright( $output, 2, 187, 82, $view['base_level']."/".$view['job_level'], $lvl_color );
			imagestringright( $output, 2, 187, 93, $view['online'] ? "Online" : "Offline", $status_color );
		...
		////////////////////////////////
		/////////////////////// AVATAR
		$border       =   imagecreatefrompng(  $view['border'] );
		$background   =   imagecreatefromjpeg( $view['background'] );
		

		// Build image
		$b_offset = 7;
		if ($view['no_border']==="true") {
			$b_offset = 0;
		}
		
		$output       =   imagecreatetruecolor( 114 + ($b_offset * 2), 114 + ($b_offset * 2));
		imagecopy( $output, $background, $b_offset, $b_offset, 0, 0, 114, 114 );
		imagecopy( $output, $img, $b_offset, $b_offset, 42, 72, imagesx($img)-($b_offset * 2), imagesx($img)-($b_offset * 2));
		if ($view['no_border']!=="true")	imagecopy( $output, $border, 0, 0, 0, 0, 128 , 128);
		...
		if( !empty($view['emblem_data']) ) {
			...
			if( $binary && ($emblem = imagecreatefrombmpstring($binary)) ) {
				...
				imagecopy( $output, $emblem, (114 + ($b_offset * 2))-10-24, (114 + ($b_offset * 2))-10-24, 0, 0, 24, 24 );
			}
		}
		...
		$status_color = $view['online'] ? imagecolorallocate($output, 59,  129,  44 ) : imagecolorallocate($output, 188,  98,  98 );

		// Draw text
		imagestring( $output, 1, 5 + $b_offset, 5 + $b_offset, strtoupper($view['name']), $name_color );
		imagestring( $output, 1, 5 + $b_offset, 18 + $b_offset, $view['base_level']."/".$view['job_level'], $lvl_color );
		imagestring( $output, 1, 74 + $b_offset, 5 + $b_offset, $view['online'] ? "ONLINE" : "OFFLINE", $status_color );
