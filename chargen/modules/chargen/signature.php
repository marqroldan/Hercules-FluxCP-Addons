...
$ava_BGDir = FLUX_ADDON_DIR."/".$this->moduleName."/data/signature/";
$signatureTMP_dir = FLUX_DATA_DIR.'/tmp/signature/';

generateTemp($signatureTMP_dir);

//$account_ava = $avatarTMP_dir.md5($session->account->account_id).'.php';
$char_ava =  $signatureTMP_dir.$name_.'.png';
$char_details = $signatureTMP_dir.$name_.'.php';

/*** Character Generation ****/
// Default Values
$char_detail = Flux::config('char_detail')->toArray();
$char_detail['bg_loc'] = $ava_BGDir;
$char_detail['background'] = Flux::config('default_sigBG');
...
elseif (file_exists($char_details)) {
	...
	if((time() - filemtime($char_ava)) > (Flux::config('ChargenCacheTime') * 60)) {
		...
		$this->redirect($this->url('chargen','index',array('saved' => $genRes, 'save_type' => 'signature', 'save_name' => $name_ )));
	}
	else {
		header('Content-type:image/png');
...
function generateImage($name_, $_post, $server, $char_detail, $char_details, $char_ava, $session, $chargen) {
	...
	if($char) {
		...
		if (count($_POST)) {
			...
			imagepng($chargen->render(1),$char_ava);
			return true;
		}
		else {
			header('Content-type:image/png');
